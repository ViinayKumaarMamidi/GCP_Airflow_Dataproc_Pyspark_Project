...
dag = DAG(
    'Ephemeral_Spark_Job_on_Dataproc_Cluster',
    default_args=default_args,
    description='A DAG to setup ephemeral dataproc cluster and run Spark job and terminate once success',
    schedule_interval=None,  # Trigger manually or on-demand
    catchup=False,
    tags=['Dev_Dataproc_Spark_Job'],
)
...
submit_pyspark_job = DataprocSubmitJobOperator(
    ...
    job={
        ...
        "pyspark_job":{
            "main_python_file_uri":"gs://XXXXX/spark-job/emp_batch_job.py"
        }
    },
    dag=dag,
)
